...
theorem growthFunction_le_sauerShelah_sum
  ...
  apply ciSup_le'
  intro S
  -- Uses: card_restrictionFamily_eq
  ...
  -- Uses: Finset.card_shatterer_le_sum_vcDim (Mathlib Sauer-Shelah)
...
/-- Bridge: (exp 1 · n/d)^d = exp(d) / (d/n)^d.
This converts the RHS of the goal into a form suitable for bounding by the binomial sum. -/
...
Proof outline:
  1. Set r = d/n ∈ (0,1]. Rewrite goal as ∑ C(n,k) ≤ exp(d) / r^d  (rpow_exp_div_form)
  2. Replace r^d by r^k (using r ≤ 1 and k ≤ d)
  3. Extend sum to full range (all terms are non-negative)
  4. Apply binomial theorem: ∑_k C(n,k)·r^k = (1+r)^n                (add_pow)
  5. Bound (1+r)^n ≤ exp(r·n) = exp(d)                               (Real.add_one_le_exp) -/
...
Uses: growthFunction_le_sauerShelah_sum, sum_choose_le_pow -/
theorem sauer_shelah_bound (C : Set (Concept X)) (m d : ℕ)
  ...
  -- Uses: growthFunction_le_sauerShelah_sum
...
/-- Arithmetic: 8/ε ≤ m with ε > 0 implies m > 0. -/
...
/-- Positivity: 1 ≤ 2·exp(1)·m/d when d ≤ 2m and m ≥ 1. -/
...
/-- The log of the growth function is bounded by d · log(2em/d).

Proof: case-split on whether growthFunction = 0.
Uses: sauer_shelah_bound, one_le_two_exp_mul_div -/
...
Proof outline:
  1. P[A] ≤ 2·P[B]                  (ghost_sample_bound)
  2. P[B] ≤ δ/2                      (sample_size_bound, log_growthFunction_le_sauerShelah)
  3. 2·(δ/2) = δ                     (two_mul_ofReal_div2) -/
